...
import { FlatList, TouchableHighlight, View, type FlatListProps as _FlatListProps } from 'react-native'
  ...
  return (
    <View style={{ ...styles.listItem, height: ITEM_HEIGHT, backgroundColor: isSelected ? theme['c-primary-background-hover'] : 'rgba(0,0,0,0)' }} onStartShouldSetResponder={() => true}>
      ...
      <View style={styles.listItemBtns}>
        <TVButton style={styles.listItemBtn} onPress={() => { onPlay(info) }} borderRadius={6} hasTVPreferredFocus={true}>
          <Icon name="play-outline" style={{ color: theme['c-button-font'] }} size={18} />
        </TVButton>
        <TVButton style={styles.listItemBtn} onPress={() => { onRemove(index) }} borderRadius={6}>
...
const Modal = forwardRef<ModalType, {}>((props, ref) => {
  ...
  return (
    <Dialog ref={dialogRef} onHide={handleHide} closeBtn={false}>
      {
        info.name
          ? (<>
