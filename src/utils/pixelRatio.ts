...
// 获取屏幕的dp，windowSizeTools 可能尚未初始化（值为0），fallback 到 Dimensions
const size = windowSizeTools.getSize()
// console.log('size', size)
const _dim = Dimensions.get('window')
let screenW = size.width || _dim.width
let screenH = size.height || _dim.height
if (screenW > screenH) {
  const temp = screenW
  screenW = screenH
  screenH = temp
...
export function getTextSize(size: number) {
  ...
  const _w = screenW || Dimensions.get('window').width
  const _h = screenH || Dimensions.get('window').height
  let scaleWidth = _w / designWidth
  let scaleHeight = _h / designHeight
  // console.log(scaleWidth, scaleHeight)
  let scale = Math.min(scaleWidth, scaleHeight, 1.3)
  // fallback: 如果 scale 为 0，使用 1（不缩放）
  if (scale === 0) scale = 1
  size = Math.floor(size * scale / (fontScale || 1))
  // console.log(size)
  return size || size
}
